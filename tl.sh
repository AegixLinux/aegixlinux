...
mkfs.fat -F32 "${selected_device_path}1"
...
# sed -i "s|^GRUB_CMDLINE_LINUX_DEFAULT=\".*\"|GRUB_CMDLINE_LINUX_DEFAULT=\"loglevel=3 quiet b=UUID=$encrypted_partition_uuid:tankluks root=UUID=$luks_container_uuid\"|" /etc/default/grub
# I think the above command was breaking the system by making it unbootable. 
# Below is the update
